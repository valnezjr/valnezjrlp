...
5. **Hover-lift do mothership-ds cortado por `overflow:hidden`** — `.ms-gallery__item`/`.ms-hover-edge` crescem no hover (`translateY(-4px) scale(1.03)`); sem respiro, esse crescimento ultrapassa qualquer ancestral que clipe (inclusive `overflow-y: auto` sozinho, que vira `overflow-x: hidden` por spec no eixo que você não pediu) e corta a borda do item. Já bateu nisso duas vezes (galeria do portfólio, galeria dentro dos cases) antes de virar regra — ver `src/index.css` § "Hover-lift do mothership-ds" (as classes `.hover-lift-room`/`.hover-lift-off`) e `docs/architecture.md` § Registro de decisões #37 antes de colocar qualquer elemento com esse hover dentro de um container que clipa.
