...
| 12 | Splash de tela cheia (`App.tsx`, `loading` state) + `Splash inline persistent ready` ao lado do Hero (`Home.tsx`, só ≥1024px) como visual de apoio, resolvendo o campo que ficava em aberto no PRD | `LogoMark` não tem uso isolado fora do `Splash` por design da própria lib (partes nascem fora de posição, esperando a montagem) — `inline`+`persistent`+`ready` fixo monta e mantém o logo com o degradê de marca girando indefinidamente (`animateTransform repeatCount="indefinite"` do próprio SVG), sem repetir a sequência de entrada já vista no Splash de tela cheia. `style={{ background: "none" }}` remove o `--bg-base`/`--bg-page` que o `.ms-splash` sempre pinta por baixo (faz sentido em tela cheia, mas aqui criava uma segunda instância de fundo dentro de uma caixa visível) — sem ele, só o SVG do `LogoMark` (sem fundo próprio) sobrepõe o `LivingBackground` real da página, flutuando ao lado do Hero sem moldura nem emenda; corrigido de 256px pra 384px (`h-96 w-96`) depois de feedback direto de que a versão inicial, com borda + `var(--radius-md)`, ficava pequena e "engaiolada". Escondido abaixo de 1024px: a dupla coluna não cabe sem apertar o texto ou criar scroll | 2026-08-05 |
...
| 15 | [preencher conforme o projeto evoluir] | | |
