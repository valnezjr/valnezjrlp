...
| 18 | Portfólio em galeria paginada (`StepIndicator` do mothership-ds v1.6.0, `showCount`, `label="Página"`) em vez do componente `Gallery` da lib | `Gallery` é um grid que cresce em **altura** (`grid-template-columns: repeat(auto-fill, minmax(220px,1fr))`, sem paginação nem scroll próprios) — incompatível com a regra de nunca criar scroll interno (§4) pra "quantos projetos forem necessários". Paginação faz a lista crescer em número de **páginas**, não altura. `StepIndicator` não existia solto da `StepModal` até este pedido — extraído na própria lib (upstream, v1.6.0) depois de auditoria do `design-system-maintainer` (achou e corrigiu uma prop `{...rest}` não repassada antes de fechar a versão) | 2026-08-06 |
...
| 20 | Itens por página da galeria (4 mobile / 6 a partir de `sm`, `Sobre.tsx`) calculados via `matchMedia` em JS, não só classes Tailwind responsivas | Paginação de verdade precisa saber quantos itens cabem por página pra calcular `totalPages` corretamente — CSS sozinho poderia esconder itens visualmente por breakpoint, mas não reduziria a contagem real de páginas do `StepIndicator` | 2026-08-06 |
| 21 | Cards da galeria com `Card` do mothership-ds, mas com `style={{padding: "var(--space-2)"}}` sobrepondo o `padding: var(--space-5)` padrão, e foto com altura fixa (48px) em vez de `aspect-ratio` | `Card` e uma foto proporcional à largura da coluna (comum em grades) são generosos demais pra uma grade densa de 4–6 itens cabendo, junto com a apresentação e o paginador, numa viewport de 600px de altura (decisão #7) — testado sem estourar em 360×640 até 1920×1080 | 2026-08-06 |
| 22 | [preencher conforme o projeto evoluir] | | |
