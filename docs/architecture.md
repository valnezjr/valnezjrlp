...
| 15 | Logo em loop (decisão #12) visível em toda largura, não só ≥1024px — `h-44 w-44`/`sm:h-56 sm:w-56` e acima do Hero (`order-first` na coluna `flex-col` do mobile) abaixo de 1024px; a partir daí volta ao tamanho e posição da decisão #12 (`lg:order-none` restaura a ordem do DOM — Hero primeiro — que em `lg:flex-row` põe a logo à direita) | Feedback direto (duas rodadas): primeiro a logo sumia por completo no mobile; corrigido pra `h-28`/`sm:h-36`, mas ainda pequena demais pra reconhecer o desenho — segundo feedback direto pediu maior, bem mais visível. Testado sem estourar scroll até 360×740 (decisão #7) mesmo no tamanho maior | 2026-08-05 |
| 16 | [preencher conforme o projeto evoluir] | | |
