...
| 15 | Logo em loop (decisão #12) visível em toda largura, não só ≥1024px — pequena (`h-28 w-28`, `sm:h-36 sm:w-36`) e acima do Hero (`order-first` na coluna `flex-col` do mobile) abaixo de 1024px; a partir daí volta ao tamanho e posição da decisão #12 (`lg:order-none` restaura a ordem do DOM — Hero primeiro — que em `lg:flex-row` põe a logo à direita) | Feedback direto: a logo sumia por completo no mobile, quando deveria só ficar menor. Testado sem estourar scroll até 360×740 (decisão #7) | 2026-08-05 |
